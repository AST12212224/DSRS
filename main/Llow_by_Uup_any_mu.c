...
int main() {
    ...
    if (choice == 'y' || choice == 'Y') {
        ...
        // Use automatic start index
        long long auto_index = find_start_index(up_up, low_low, l-1);
        if (auto_index == l-1) {
            printf("No valid non-zero starting index found. Product is undefined.\n");
            free(up); free(low); free(up_up); free(low_low);
            return 0;
        }
        start_index_up = auto_index;
        start_index_low = auto_index;
    }
